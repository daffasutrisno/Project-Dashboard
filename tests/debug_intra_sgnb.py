"""
Debug script untuk melihat detail data Intra sgNB intrafreq per tanggal
"""
...
def check_intra_sgnb_data():
    """Check Intra sgNB intrafreq data untuk 35 hari terakhir"""
    ...
    print("="*70)
    print("INTRA SGNB INTRAFREQ DATA DEBUG - 35 HARI TERAKHIR")
    print("="*70)
    print(f"\nDate range: {min_date.strftime('%Y-%m-%d')} to {max_date.strftime('%Y-%m-%d')}")
    print(f"Total records: {len(df)}")
    
    # Group by date (MAX for intra_sgnb_intrafreq)
    daily = df.groupby('date_column').agg({
        'intra_sgnb_intrafreq': ['count', 'max', 'min', 'mean']
    }).reset_index()
    ...
    if len(valid_days) > 0:
        ...
        print(f"\nIntra sgNB intrafreq Statistics (MAX per day):")

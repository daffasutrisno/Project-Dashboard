"""
Debug script untuk melihat detail data Intra esgNB per tanggal
"""
...
def check_intra_esgnb_data():
    """Check Intra esgNB data untuk 35 hari terakhir"""
    ...
    print("="*70)
    print("INTRA ESGNB DATA DEBUG - 35 HARI TERAKHIR")
    print("="*70)
    print(f"\nDate range: {min_date.strftime('%Y-%m-%d')} to {max_date.strftime('%Y-%m-%d')}")
    print(f"Total records: {len(df)}")
    
    # Group by date (MAX for intra_esgnb)
    daily = df.groupby('date_column').agg({
        'intra_esgnb': ['count', 'max', 'min', 'mean']
    }).reset_index()
    ...
    if len(valid_days) > 0:
        ...
        print(f"\nIntra esgNB Statistics (MAX per day):")

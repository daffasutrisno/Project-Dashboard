"""
Debug script untuk melihat detail data Inter sgNB intrafreq per tanggal
"""
...
def check_inter_sgnb_data():
    """Check Inter sgNB intrafreq data untuk 35 hari terakhir"""
    ...
    print("="*70)
    print("INTER SGNB INTRAFREQ DATA DEBUG - 35 HARI TERAKHIR")
    print("="*70)
    print(f"\nDate range: {min_date.strftime('%Y-%m-%d')} to {max_date.strftime('%Y-%m-%d')}")
    print(f"Total records: {len(df)}")
    
    # Group by date (MAX for inter_sgnb_intrafreq)
    daily = df.groupby('date_column').agg({
        'inter_sgnb_intrafreq': ['count', 'max', 'min', 'mean']
    }).reset_index()
    ...
    if len(valid_days) > 0:
        ...
        print(f"\nInter sgNB intrafreq Statistics (MAX per day):")
